...
Collection is on from the first load (since 0.1.14). Since 0.1.15 the addon replaces the player's name, race and class in all captured text with `<name>`, `<race>` and `<class>` before saving, and marks those records (`textSchema = 1`); only marked records upload their text. Text saved by older versions still holds the name and stays local. Quest text is Blizzard's game content; RestedRealm shows it with attribution once two players report the same words. The addon holds at most 5,000 records (1,500 before 0.1.16) and stops recording when full, preserving earlier observations. RestedRealm Companion empties the list each time WoW is fully closed, once every record is safely in its queue. It does not intentionally collect chat, player identities, account IDs, full inventory, credentials or continuous movement history. Locations are the **player's point at interaction**, not a proven exact NPC or object spawn. `loot_window` means an item was visible in a loot window, not that it was acquired. Some IDs or fields may be absent until verified in this Forever build.
