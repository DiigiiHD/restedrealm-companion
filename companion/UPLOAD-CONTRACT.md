...
- Text (quest text, objectives, NPC dialogue, greetings, gossip option names) travels only in records marked `textSchema: 1`, where the addon already replaced the player's name, race and class with `<name>`, `<race>` and `<class>`. The server refuses text in unmarked records, allows up to 8,192 characters per text field, and always refuses `tooltipProbe`. Owner decision of 25 September 2026: quest text is shown on the site as Blizzard's content, with attribution, once two accounts agree on the words for a game version.
