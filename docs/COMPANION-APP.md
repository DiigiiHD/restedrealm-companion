...
4. **Daily use.** The app starts minimized to the icon next to the Windows clock. The icon's menu shows status, "Upload now", "Open RestedRealm", "Settings" and "Quit". The main window shows connection, the last upload, the queue count and a short history. Notifications are rare: only a problem the player can fix, such as a lost connection or a WoW folder that moved.
